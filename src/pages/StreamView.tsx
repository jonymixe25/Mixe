...
const StreamView = () => {
  ...
  const [moderationSensitivity, setModerationSensitivity] = useState<
    "low" | "medium" | "high"
  >("medium");
  ...
  const acceptInvitation = async () => {
    ...
    try {
      await updateDoc(
        doc(db, "streams", id, "invitations", incomingInvitation.id),
        {
          status: "accepted",
  ...
  const {
    ...
    url: liveKitUrl,
    error: tokenError,
  } = useLiveKitToken(id || "", viewerIdentity);
  ...
  useEffect(() => {
    if (tokenError) {
      ...
      setToast({
        message: `Error de señal: ${tokenError}`,
        type: "error",
        isVisible: true,
      });
    }
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        if (snapshot.exists()) {
          setModerationSensitivity(
            snapshot.data().moderationSensitivity || "medium",
          );
        }
      },
      (error) => {
  ...
  useEffect(() => {
    ...
    if (user) {
      unsubscribeJoinRequest = onSnapshot(
        doc(db, "streams", id, "joinRequests", user.uid),
        (doc) => {
          if (doc.exists()) {
    ...
    const setupLiveKit = async () => {
      ...
      room.on(
        ...
        (track: Track, publication: any, participant: any) => {
              ...
              setActiveVideoTracks((prev) => {
                ...
                return [
                  ...prev,
                  {
                    ...
                    track,
                    name: participant.name || participant.identity || "Anfitrión",
                    role: participant.identity === stream?.userId ? "host" : "guest",
                  },
                ];
              });
            }
          } else if (track.kind === Track.Kind.Audio) {
            // Keep the standard audio attachment to play sound
    ...
    return () => {
      ...
      updateViewerCount(-1);
      setActiveVideoTracks([]);
      if (roomRef.current) {
        roomRef.current.disconnect();
        roomRef.current = null;
      }
    };
  }, [id, navigate, token, liveKitUrl, user, stream?.userId]);
      ...
      const publishLocalCamera = async () => {
        try {
          ...
          try {
            ...
            setToast({
              message: "No se pudo acceder al video, transmitiendo solo audio",
              type: "error",
              isVisible: true,
            });
          }
          ...
          if (videoTrack) {
            ...
            const trackInstance = pub.track;
            if (trackInstance) {
              setActiveVideoTracks((prev) => {
                if (prev.some((t) => t.id === trackInstance.sid)) return prev;
                return [
                  ...prev,
                  {
                    id: trackInstance.sid || "local-video",
                    track: trackInstance,
                    name: user?.displayName || "Tú (Invitado)",
                    role: "me",
                  },
                ];
              });
          ...
          setToast({
            message: "Estás transmitiendo en duo",
            type: "success",
            isVisible: true,
          });
        } catch (error) {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    if (!user) {
      setToast({
        message: "Inicia sesión para participar en el chat",
        type: "error",
        isVisible: true,
      });
      return;
    ...
    try {
      ...
      const sensitivityPrompt =
        moderationSensitivity === "high"
          ? "Sé extremadamente estricto: bloquea cualquier mensaje que pueda ser remotamente ofensivo, spam, o que use lenguaje informal inapropiado."
          : moderationSensitivity === "low"
            ? "Sé permisivo: bloquea solo insultos graves o spam evidente."
      ...
      if (response.text?.trim().toUpperCase() === "SI") {
        setToast({
          message: "Mensaje bloqueado por moderación automática.",
          type: "error",
          isVisible: true,
        });
        return;
  ...
  const handleChatImageUpload = async (
    ...
    if (!user) {
      setToast({
        message: "Inicia sesión para enviar imágenes",
        type: "error",
        isVisible: true,
      });
      return;
  ...
  if (loading)
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#070504] -mx-4 md:-mx-8 lg:-mx-12 -mt-8">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-brand"></div>
      </div>
    );

  return (
    <div className="min-h-screen bg-[#070504] text-white selection:bg-brand/30 -mx-4 md:-mx-8 lg:-mx-12 -mt-8 overflow-hidden">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-2 mb-0.5">
                <div className="w-1.5 h-1.5 bg-brand rounded-full animate-pulse shadow-[0_0_8px_#ff4e00]" />
                <span className="text-[9px] font-semibold uppercase tracking-[0.15em] text-brand">
                  EN VIVO
                </span>
                {stream?.privacy === "private" && (
                  <div className="flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-yellow-500/10 border border-yellow-500/20">
                    ...
                    <span className="text-[8px] font-bold uppercase text-yellow-500/70">
                      PRIVADO
                    </span>
                  </div>
                )}
              </div>
              <h1 className="text-sm font-bold tracking-tight text-white/90 line-clamp-1">
                {stream?.title}
              </h1>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button
                ...
                className={`p-2.5 rounded-full border transition-all ${showStats ? "bg-brand/20 border-brand/20 text-brand" : "bg-white/5 border-white/5 text-white/50"}`}
                title="Telemetría"
              >
        ...
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
          ...
          <div className="flex-1 relative bg-black flex items-center justify-center overflow-hidden group/player">
              ...
              {activeVideoTracks.length === 0 ? (
                ...
                <div className="flex flex-col items-center justify-center text-center p-6 bg-black/40 rounded-3xl border border-white/5 backdrop-blur-md max-w-sm">
                  ...
                  <p className="text-white/40 text-[10px] leading-relaxed">
                    Estableciendo una señal segura de video y audio en tiempo real...
                  </p>
                </div>
                        ...
                        <div className="w-full h-full relative flex items-center justify-center">
                          ...
                          {pipTrack && (
                            <div
                              ...
                              className="absolute bottom-4 right-4 w-28 sm:w-48 aspect-[4/3] bg-zinc-900 border-2 border-white/20 hover:border-brand shadow-2xl rounded-xl overflow-hidden transition-all duration-300 hover:scale-105 active:scale-95 cursor-pointer group/pip text-left z-40"
            ...
            <div className="absolute inset-0 pointer-events-none z-30 overflow-hidden">
              <AnimatePresence>
                {reactions.map((r) => (
                  ...
                  >
                    <Heart className="w-10 h-10 text-brand fill-current drop-shadow-[0_0_15px_#ff4e00]" />
                  </motion.div>
                ))}
              </AnimatePresence>
            </div>
            ...
            <AnimatePresence>
              {stream?.status === "ended" && (
                ...
                >
                  ...
                  <h2 className="text-2xl font-bold uppercase tracking-tighter mb-4 italic">
                    Transmisión Finalizada
                  </h2>
                  <p className="text-white/50 text-xs mb-10 max-w-xs">
                    Esperamos volver a verte pronto para más cultura Mixe.
                  </p>
                  <button
                    onClick={() => navigate("/")}
                    className="px-8 py-3.5 bg-brand text-black rounded-xl font-semibold uppercase tracking-wider text-[10px] hover:scale-105 transition-all shadow-lg shadow-black/[0.03] shadow-black/[0.04] shadow-[#ff4e00]/20"
                  >
                    Volver al Inicio
                  </button>
                </motion.div>
              ...
              {connectionStatus !== "connected" && (
                ...
                >
                  ...
                  <p className="mt-6 text-[10px] font-semibold uppercase tracking-[0.15em] text-brand animate-pulse">
            ...
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-[60] flex items-center transition-all duration-300">
              <div className="bg-black/80 backdrop-blur-2xl px-6 py-3 rounded-full border border-white/10 flex items-center gap-6 shadow-lg shadow-black/[0.03] shadow-black/[0.04]">
                <button
                  onClick={togglePlay}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="w-20 h-1 bg-white/10 rounded-full overflow-hidden relative cursor-pointer group/seek">
                    ...
                    />
                    <div
                      className="h-full bg-white/5 group-hover/seek:bg-brand transition-colors"
                ...
                <div className="flex items-center gap-4">
                  <button
                    onClick={handleLike}
                    className={`flex items-center gap-2 text-[10px] font-bold tracking-widest transition-colors ${isLiked ? "text-brand" : "text-white/50 hover:text-white/60"}`}
                  >
                    <Heart
          ...
          <aside className="w-full lg:w-[380px] bg-[#0a0a0a] border-l border-white/5 flex flex-col h-[40vh] lg:h-full relative z-50">
            ...
            <div className="p-5 border-b border-white/5 flex items-center gap-4 bg-white/[0.02]">
              ...
              {user && user.uid !== stream?.userId && joinStatus === "none" && (
                <button
                  ...
                  className="ml-auto p-2 rounded-lg bg-white/5 border border-white/10 hover:bg-brand/20 hover:border-brand/20 transition-all group"
                  title="Unirse en Dúo"
                >
                  <UserPlus className="w-4 h-4 text-white/40 group-hover:text-brand" />
                </button>
              )}
            </div>

            {/* Sidebar Tabs */}
            <div className="flex items-center border-b border-white/5 shrink-0 bg-white/[0.02]0">
              <button className="flex-1 py-3.5 text-[9px] font-semibold uppercase tracking-[0.15em] border-b-2 border-brand text-white">
                Chat
              </button>
              <button className="flex-1 py-3.5 text-[9px] font-semibold uppercase tracking-[0.15em] border-b-2 border-transparent text-white/30 hover:text-white/50 transition-colors">
                Vivos
              </button>
            </div>
      ...
      <AnimatePresence>
        {isInvitationModalOpen && incomingInvitation && (
          <Modal
            ...
            onClose={rejectInvitation}
            title="Invitación a Dúo"
          >
            <div className="text-center space-y-6">
              <div className="w-20 h-20 bg-brand/10 rounded-full flex items-center justify-center mx-auto mb-4 border border-brand/20">
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className="flex-1 px-6 py-3 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all uppercase text-[10px] font-bold tracking-widest"
                >
                  Rechazar
                </button>
                <button
                  onClick={acceptInvitation}
                  className="flex-1 px-6 py-3 rounded-xl bg-brand text-black hover:bg-brand/90 transition-all uppercase text-[10px] font-bold tracking-widest shadow-lg shadow-black/[0.03] shadow-[#ff4e00]/20"
                >
                  Aceptar
                </button>
              </div>
